        ...
        match op {
            ...
            Op::Eq => {
                self.pop().and_then(|x| {
                    self.pop().and_then(|y| {
                        match (x,y) {
                            (Elt::Int(x),Elt::Int(y)) => self.push(Elt::Bool(x==y)),
                            (Elt::Bool(x),Elt::Bool(y)) => self.push(Elt::Bool(x==y)),
                            _ => Err(Error::Type),
                        }
                    })
                })
            ...
            Op::Rand => {
                ...
                self.pop().and_then(|x| {
                    match x {
                        Elt::Int(x) => self.push(Elt::Int(rng.gen_range(0,x))),
                        _ => Err(Error::Type),
                    }
                })
            },
