    ...
    loop {
        ...
        for line in io::stdin().lock().lines() {
            match line {
                ...
                Err(io_err) => return Err(rpn::Error::IO(io_err)),
            }
        }
    }
}
...
fn evaluate_line(stack: &mut Stack, buf: &String) -> rpn::Result<()> {
    ...
    tokens.map(|token| -> rpn::Result<()> {
        match token {
            ...
            _ => {
                match i32::from_str_radix(token,10) {
                    Ok(x) => stack.push(rpn::Elt::Int(x)),
                    Err(_) => Err(rpn::Error::Syntax),
                }
            },
        }
    }).collect::<rpn::Result<Vec<()>>>().map(|_| ())
}
